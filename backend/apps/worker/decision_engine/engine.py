    ...
    def evaluate(self, signal: Any, snapshot: MarketSnapshot) -> DecisionResult:
        ...
        if invalid:
            ...
            return self._finalize(
                Decision.REJECT, 0, 0, 0, int(self.settings.decision_threshold), reasons, metrics
            )
        ...
        vol_hard = rules.check_volatility_hard(
            ...
            min_dist=(
                ...
                if self.settings.atr_stop_hard_min is not None
                else 0.3
            ),
            max_dist=(
                float(self.settings.atr_stop_hard_max)
                if self.settings.atr_stop_hard_max is not None
                else 5.0
            ),
        )
        if vol_hard:
        ...
        s, r = rules.score_volatility(
            ...
            min_soft=(
                ...
                if self.settings.atr_stop_soft_min is not None
                else 0.6
            ),
            max_soft=(
                float(self.settings.atr_stop_soft_max)
                if self.settings.atr_stop_soft_max is not None
                else 2.5
            ),
            max_score=w_vol,
        )
        ...
        if signal.side == "BUY":
            ...
            recent_lows = [low for low in lows[-window:] if low < entry_val]
            if recent_lows:
        ...
        reasons.append(
            Reason(
                code=ReasonCode.LIQUIDITY_UNKNOWN,
                severity=Severity.WARN,
                msg="Liquidity assumed (Stub)",
            )
        )
        ...
        return self._finalize(
            decision, score_pct, score_raw, score_max, threshold_pct, reasons, metrics
        )
