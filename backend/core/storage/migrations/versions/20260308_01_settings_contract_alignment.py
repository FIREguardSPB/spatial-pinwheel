...
def upgrade() -> None:
    ...
    op.add_column('settings', sa.Column('ai_fallback_providers', sa.String(), nullable=True, server_default='ollama,skip'))
    ...
    op.execute("UPDATE settings SET ai_fallback_providers = 'ollama,skip' WHERE ai_fallback_providers IS NULL")
