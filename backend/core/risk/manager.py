from core.models.schemas import Signal
from core.storage.models import Position, Settings
...
class RiskManager:
    ...
    def check_new_signal(self, signal: Signal) -> tuple[bool, str]:
        ...
        active_positions = self.db.query(
            Position
        ).count()  # Simply count all rows in positions table?
        ...
        if active_positions >= self.settings.max_concurrent_positions:
            return (
                False,
                f"Max positions reached ({active_positions}/{self.settings.max_concurrent_positions})",
            )
