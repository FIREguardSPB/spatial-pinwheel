...
- Запускает API (порт 8000), Worker (фоновая торговля), Frontend (порт 5173)
...
**Внимание:** Реальная торговля через T-Bank gRPC ещё не реализована (P2-09). Все ордера исполняются как бумажные.
...
Обязательно заполнить: `AUTH_TOKEN`, `POSTGRES_PASSWORD`, `APP_ENV=prod`.
